...
function AzureActiveDirectoryValidationManager() {
    ...
    /*
     * Extracts the version from the give jwt token
     */
     ...
     */
    self.requestOpenIdConfig = function(tenantId, version, cb) {
        // we need to load the tenant specific open id config
        var tenantOpenIdconfig = {
          url: version.startsWith('1')
            ? 'https://login.windows.net/' + tenantId + '/.well-known/openid-configuration'
            : 'https://login.windows.net/' + tenantId + '/v2.0/.well-known/openid-configuration',
          json: true
        };
     ...
     */
    self.verify = function(jwt, certificates, options, version, cb) {
        ...
        options.issuer = version.startsWith('1') 
            ? 'https://sts.windows.net/' + self.getTenantId(jwt) + '/'
            : 'https://login.microsoftonline.com/' + self.getTenantId(jwt) + '/v2.0';
